...
broad-width distribution distance improves from 0.41 to 0.32 with the
widths reinterpreted as substantially electron-scattering broadened
($\tau_{\rm e}\simeq2.0$, contributing comparably to the virial term). The exponent is not fitted but constructed and
...
\citep{Matthee2024,Greene2024,Akins2024,Kocevski2025}. Three
...
Clustering analyses additionally bound the host halos
\citep{Pizzati2025,Carranza2025}, and assembly-bias clustering
diagnostics of the formation channel have been proposed
\citep{Wang2026}---constraints the lifecycle's
halo occupation must ultimately face. Our model shares the
dense-envelope architecture of the black-hole-star models above and the
low-spin entry channel proposed for
...
\startdata
xLRD & 1.20 & $1.10\pm0.10$ & 0.99 & 0.981 (0.978--0.983)\\
plusLRD & 3.00 & $3.20\pm0.10$ & 0.94 & 0.901 (0.884--0.911)\\
minusLRD & 3.98 & $4.40\pm0.30$ & 0.79 & 0.738 (0.625--0.807)\\
bLRD & 4.59 & $4.90\pm0.50$ & 0.47 & 0.086 (0.000--0.658)\\
\enddata
...
distance 0.325 versus 0.413; a descriptive statistic---no null
...
black-hole masses (median $\log M_{\rm BH}$ 6.85$\rightarrow$7.26) and
Compton-thick Thomson depths ($\tau_{\rm e}\simeq2.0$), so
$\simeq1550~{\rm km\,s^{-1}}$ of the width budget is electron-scattering
broadening, comparable to the virial term at the fitted
scale---consistent with envelope models in which scattering shapes
the Balmer profiles \citep{Naidu2025,deGraaff2025}---and the descriptive
compact scale moves from $1.2\times10^{3}$ to $6.9\times10^{3}$ AU
(bootstrap 6.3--8.5$\times10^{3}$; the scan support was widened from the
published 5000 AU edge to keep this fit interior, which we disclose as a
boundary effect of the original support). The factor-of-six scale shift
...
scattering halo ($\gtrsim7\times10^{3}$ AU) is required for
...
$\simeq1550~{\rm km\,s^{-1}}$ budget is unchanged; what the relocation
...
observed median at $6.9\times10^{3}$ AU with a substantial
...
the ordering is claimed; published LRD monitoring and light-curve
analyses find low variability
...
effective-area masks. Both are measurable model-independently: survey
depth is an exclusion restriction---it changes detection without changing
the source---so re-running the color selection on the same field imaged,
or artificially degraded, to two depths measures the gate's completeness
directly. Applied to JADES SED properties, this cross-depth technique
shows that selection bias near the detection limit is observable-dependent
and flips sign at the Balmer break \citep{PandevXDC}---the wavelength at
which the V-shape gate operates---independently supporting the conclusion
that color-gated LRD demography is selection-dominated.

\section{Limitations}\label{sec:limits}
The observation layer remains a set of transparent proxies: the selection
gates are not the survey pipelines (though cross-depth reselection offers
a model-independent calibration path; \citealt{PandevXDC}); the X-ray
transfer over-suppresses
...
accompany this paper as an MIT-licensed code repository at
\url{https://github.com/spsingularity/lrd-covering-law}, together with
machine-readable results for every table and figure and a single
reproduction script that regenerates all results and both manuscripts
(v1.0.0 archived at Zenodo, doi:10.5281/zenodo.21475322); the full
67-configuration forward-catalogue grid
is deposited as a separate CC-BY-4.0 data record
(doi:10.5281/zenodo.21476542). Only
...
Narayan, R. 2024, ApJ, 976, 96
\bibitem[Pandev(2026d)]{PandevXDC} Pandev, S. 2026d, XDC: the
observable dependence of correlated selection bias in JWST
high-redshift galaxies, Zenodo, doi:10.5281/zenodo.21400943
...
arXiv:2604.07138
\bibitem[Rusakov et al.(2026)]{Rusakov2025} Rusakov, V., et al. 2026,
Nature, 649, 574
